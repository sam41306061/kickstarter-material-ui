...
async function main() {
    ...
    const Campaign1 = await ethers.getContractAt('Campaign','0x5FbDB2315678afecb367f032d93F642f64180aa3');
    console.log(`Campign 1 as fetched at: ${Campaign1.address}`);

    const Campaign2 = await ethers.getContractAt('Campaign','0x8464135c8F25Da09e49BC8782676a84730C318bC');
    console.log(`Campign 2 as fetched at: ${Campaign2.address}`);

    const Campaign3 = await ethers.getContractAt('Campaign','0x663F3ad617193148711d28f5334eE4Ed07016602');
    ...
    console.log(`Contributing to Campaign 1 with a total of  ${amount} from ${contributer.address} to ${owner.address} campaign\n`);
 ...
 /////////////////////////////////////////////////////////////
  ...
  //
    ...
    transaction = await Campaign2.connect(contributer1).contribute({ value: amount });
    await transaction.wait();
    console.log(`Contributed ${amount} from \n ${owner.address} \n to ${Campaign2.address} campaign\n`);
    
    //contribute to campaign 3 approved by contributer2
    transaction = await Campaign3.connect(contributer2).contribute({ value: amount });
    await transaction.wait();
    console.log(`Contributed ${amount} from \n ${owner.address}\n to ${Campaign3.address} campaign\n`);

    //contribute to campaign 1 approved by contributer1
    transaction = await Campaign1.connect(contributer1).contribute({ value: amount });
    await transaction.wait();
  ...
  transaction = await Campaign1.connect(owner).createRequest('Buy a car', contribution(100), owner.address);
  result = await transaction.wait();
  console.log(`Created request for ${owner.address} to ${Campaign1.address} campaign\n`);
